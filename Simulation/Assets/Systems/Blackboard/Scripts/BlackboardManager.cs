...
public enum EBlackboardKey
{
    ...
    Character_CurrentTaskType, // 作業カテゴリ（例："Work", "Rest"）
    Character_CurrentSmartObject,
...
public class BlackboardManager : MonoBehaviour
{
    ...
    {
        // 一度壊れた参照を強制的に除去
        List<MonoBehaviour> toRemove = new();
        foreach (var key in IndividualBlackboards.Keys)
        {
            if (key == null) toRemove.Add(key);
        }
        foreach (var rem in toRemove)
            IndividualBlackboards.Remove(rem);
    ...
    {
        IndividualBlackboards.Clear();
        SharedBlackboards.Clear();
        Debug.Log("BlackboardManager: Cleared all blackboards.");
    }
    
    public void RemoveBlackboard(MonoBehaviour target)
    {
        if (IndividualBlackboards.ContainsKey(target))
        {
            IndividualBlackboards.Remove(target);
            Debug.Log($"BlackboardManager: Removed blackboard for {target.name}");
        }
    }
